...
void CuEpConfigActivity::execute()
{
    ...
    ctrl_data[TTT::Properties] = std::vector<std::string>();  // ctrl_data["properties"]
    ctrl_data[TTT::Mode] = "caget";  // ctrl_data["mode"]
    CuEpicsWorld utils;
    ...
    utils.caget(ctrl_data[TTT::Pv].toString(), ctrl_data);  // ctrl_data["pv"]

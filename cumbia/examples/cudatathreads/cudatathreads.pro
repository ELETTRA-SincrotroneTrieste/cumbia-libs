...
!isEmpty(INSTALL_DIR) {
    ...
    message("       execute `make install` to install 'cudatathreads' under $${INSTALL_DIR} ")
    message("-")
} else {
    message("-")
    message("INSTALLATION")
    message("       call qmake INSTALL_DIR=/usr/local/bin to install cudatathreads later with `make install` ")
    message("-")
}

# unix:LIBS += -L. -lmylib

unix:LIBS += -lcumbia -L/usr/local/cumbia-libs/lib

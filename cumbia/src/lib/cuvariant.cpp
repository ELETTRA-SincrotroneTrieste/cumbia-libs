...
CuVariantPrivate::CuVariantPrivate(const CuVariantPrivate &other) {
    ...
    case CuVariant::Double: {
        if(format == CuVariant::Scalar || format == CuVariant::Vector) {
            ...
            memcpy(vd, other.val, sizeof(double) * mSize);
            val = vd;
        }
    ...
    case CuVariant::Int:
    {
        if(format == CuVariant::Scalar || format == CuVariant::Vector) {
            ...
            memcpy(vi, other.val, sizeof(int) * mSize);
            val = vi;
        }
        else if(format == CuVariant::Matrix )
    ...
    case CuVariant::UInt:  {
        if(format == CuVariant::Scalar || format == CuVariant::Vector) {
            ...
            memcpy(vi, other.val, sizeof(unsigned int) * mSize);
            val = vi;
        }
        else if(format == CuVariant::Matrix )
    ...
    case CuVariant::LongInt:  {
        if(format == CuVariant::Scalar || format == CuVariant::Vector) {
            long int  *vli = new long int [mSize];
            memcpy(vli, other.val, sizeof(long int) * mSize);
            val = vli;
        }
        else if(format == CuVariant::Matrix )
    ...
    case CuVariant::LongUInt:  {
        if(format == CuVariant::Scalar || format == CuVariant::Vector) {
            unsigned long int  *vuli = new unsigned long int [mSize];
            memcpy(vuli, other.val, sizeof(unsigned long int) * mSize);
            val = vuli;
        }
        else if(format == CuVariant::Matrix )
    ...
    case CuVariant::LongLongInt:  {
        if(format == CuVariant::Scalar || format == CuVariant::Vector) {
            long long int  *vlli = new long long int [mSize];
            memcpy(vlli, other.val, sizeof(long long int) * mSize);
            val = vlli;
        }
        else if(format == CuVariant::Matrix )
    ...
    case CuVariant::LongLongUInt:  {
        if(format == CuVariant::Scalar || format == CuVariant::Vector) {
            unsigned long long int  *vulli = new unsigned long long int [mSize];
            memcpy(vulli, other.val, sizeof(unsigned long long int) * mSize);
        }
        else if(format == CuVariant::Matrix )
    ...
    case CuVariant::Boolean: {
        if(format == CuVariant::Scalar || format == CuVariant::Vector) {
            ...
            memcpy(vb, other.val, sizeof(bool) * mSize);
            val = vb;
        }
        else if(format == CuVariant::Matrix )
    ...
    case CuVariant::Short:  {
        if(format == CuVariant::Scalar || format == CuVariant::Vector) {
            ...
            memcpy(vs, other.val, sizeof(short) * mSize);
            val = vs;
        }
        else if(format == CuVariant::Matrix )
    ...
    case CuVariant::UShort: {
        if(format == CuVariant::Scalar || format == CuVariant::Vector) {
            ...
            memcpy(vus, other.val, sizeof(unsigned short) * mSize);
            val = vus;
        }
        else if(format == CuVariant::Matrix )
    ...
    case CuVariant::Char:  {
        if(format == CuVariant::Scalar || format == CuVariant::Vector) {
            ...
            memcpy(vc, other.val, sizeof(char) * mSize);
            val = vc;
        }
        else if(format == CuVariant::Matrix )
    ...
    case CuVariant::UChar: {
        if(format == CuVariant::Scalar || format == CuVariant::Vector) {
            ...
            memcpy(vuch, other.val, sizeof(unsigned char) * mSize);
            val = vuch;
        }
        else if(format == CuVariant::Matrix )
    ...
    case CuVariant::Float: {
        if(format == CuVariant::Scalar || format == CuVariant::Vector) {
            ...
            memcpy(vf, other.val, sizeof(float) * mSize);
            val = vf;
        }
        else if(format == CuVariant::Matrix )
...
{
    if(_d->val != nullptr && _d->format != Matrix)
    {
        /*
...
// alloc CuVariantPrivate if _d ptr is null
// otherwise unref _d and make a deep copy
...
CuVariant::CuVariant(const std::vector<bool> &vb) {
    ...
    // vb.data not implemented for bool
    _d->mSize = vb.size(); _d->nrows = 1; _d->ncols = 0;
    bool* pv = (bool *) new bool[_d->mSize];
    for(size_t i = 0; i < _d->mSize; i++)
        pv[i] = vb[i];
    _d->val = pv;
    _d->mIsNull = false;
 ...
 *
 * \note internal data storage is not initialized, so that the size of an empty
 *       CuVariant is that of a sole pointer and no memory allocation takes place
 */
CuVariant::CuVariant() {
    _d = nullptr;
}
 ...
 *
 * If the other variant is valid, the contents are copied (see note).
 * Otherwise, the variant remains null, no memory is allocated.
 *
 * \note rather than being deep copied, data is implicitly shared
 *
...
bool CuVariant::operator ==(const CuVariant &other) const {
    if(!_d)
        return other._d == nullptr;
    else if(other._d) {
        // both _d and other._d not null
        ...
        char **v_str = nullptr, **other_v_str = nullptr;
        /*
...
CuVariant::DataFormat CuVariant::getFormat() const {
    if(!_d) return CuVariant::FormatInvalid;
...
CuVariant::DataType CuVariant::getType() const {
    if(!_d) return CuVariant::TypeInvalid;
 ...
 *
 * \note
 * isValid may return true if data is nullptr. In other words, nullptr values
 * are deemed valid.
 *
 */
bool CuVariant::isValid() const {
    return _d && _d->mIsValid;
}

/** \brief Returns whether the the data stored by CuVariant is nullptr or not
 *
 * @return true the data contained by CuVariant is nullptr
 * @return false the data contained by CuVariant is not nullptr (see isValid)
 *
 * @see getError
 * @see isValid
 * @see isWNull
...
bool CuVariant::isUnsignedType() const {
    return _d &&
           (_d->type == UShort || _d->type ==  UInt ||
            _d->type ==  LongUInt || _d->type == UChar ||
            _d->type == LongLongUInt);
}

bool CuVariant::isSignedType() const {
    return _d && (_d->type == Short || _d->type == Int || _d->type == LongInt || _d->type == LongLongInt || _d->type == Char);
}
...
bool CuVariant::isFloatingPoint() const {
    return _d && (_d->type == Double || _d->type == LongDouble
                  || _d->type == Float);
}
...
//
// called from constructors that have allocated _d and set mIsValid, data type and format
//
template<typename T>
void CuVariant::m_from(const std::vector<T> &v) {
    ...
    memcpy(_d->val, v.data(), sizeof(T) * _d->mSize);
    _d->mIsNull = false;
}
 ...
 * - isNull is set to false
 *
 * Called from constructors (_d not null, format and type known)
 */
template<typename T>
 ...
 *        format, invalid CuVariant or nullptr value)
 * @return the double value held in this object, or *NaN* if either the data type
 *         or format is wrong, the CuVariant is not valid or the value is nullptr
 */
double CuVariant::toDouble(bool *ok) const {
    double v = nan("NaN");
    bool can_convert = _d && (_d->type == Double && _d->format == Scalar && _d->val != nullptr && _d->mIsValid);
    if(can_convert) {
            v = *((double *)_d->val);
    }
    if(ok)
...
long double CuVariant::toLongDouble(bool *ok) const {
    ...
    bool can_convert = _d && (_d->type == LongDouble && _d->format == Scalar && _d->val != nullptr && _d->mIsValid);
    if(can_convert) {
...
long int CuVariant::toLongInt(bool *ok) const {
    long int i = LONG_MIN;
    bool canConvert = _d && (_d->type == Int || _d->type == LongInt)  && _d->format == Scalar && _d->val != nullptr && _d->mIsValid;
    if(canConvert) {
        if(_d->format == Scalar && _d->val != nullptr && _d->type == Int)
...
#endif
    bool canConvert = _d && (_d->type == Int || _d->type == LongInt || _d->type == LongLongInt)  && _d->format == Scalar && _d->val != nullptr && _d->mIsValid;
    if(canConvert && _d->format == Scalar && _d->val != nullptr && _d->type == Int)
...
int CuVariant::toInt(bool *ok) const {
    int i = INT_MIN;
    bool canConvert = _d && (_d->type == Int || _d->type == Short)  && _d->format == Scalar && _d->val != nullptr && _d->mIsValid;
...
unsigned int CuVariant::toUInt(bool *ok) const {
    ...
    bool can_convert = _d && (_d->format == Scalar && _d->val != nullptr  && _d->mIsValid && (_d->type == UInt || _d->type == UShort) );
...
#endif
    bool can_convert = _d && (_d->format == Scalar && _d->val != nullptr  && _d->mIsValid &&
...
unsigned short CuVariant::toUShortInt(bool *ok) const {
    ...
    bool canConvert = _d && _d->type == UShort && _d->format == Scalar && _d->val != nullptr && _d->mIsValid;
    if(canConvert )
...
short CuVariant::toShortInt(bool *ok) const {
    short s = SHRT_MIN;
    bool canConvert = _d && _d->type == Short && _d->format == Scalar && _d->val != nullptr && _d->mIsValid;
    if(canConvert)
...
float CuVariant::toFloat(bool *ok) const {
    float f = nanf("NaN");
    bool canConvert = _d && _d->type == Float && _d->format == Scalar && _d->val != nullptr && _d->mIsValid;
    if(canConvert)
...
unsigned long int CuVariant::toULongInt(bool *ok) const {
    ...
    bool canConvert = _d && (_d->type == UInt || _d->type == LongUInt)  && _d->format == Scalar && _d->val != nullptr && _d->mIsValid;
...
bool CuVariant::toBool(bool *ok) const {
    bool b = false;
    bool can_convert = _d &&  (_d->type == Boolean && _d->format == Scalar && _d->val != nullptr && _d->mIsValid);
    if(can_convert)
...
std::string CuVariant::toString(bool *ok, const char *format) const {
    ...
    bool success = _d && _d->mSize > 0;
    memset(converted, 0, sizeof(char) * (MAXLEN + 1));
    for(size_t i = 0; _d && (_d->format == Scalar || _d->format == Vector) &&  i < _d->mSize && success; i++) { // check _d != nullptr
        if(i > 0)
            ret += ",";
        if(_d->type == String)
        ...
        if(_d->type != String) {
            if(_d->format == Vector && i == 0)
                ret.reserve(((strlen(converted) + 1) * _d->mSize) * 1.10); // +1: ','
...
/** \brief Returns the pointer to the data stored as double, or nullptr if no data is stored
 *         or if the data type is wrong
 *
 * Combined with getSize allows to get the stored data in a "C" style (as scalar, if
 * size is 1, or array).
 *
 * @return pointer to the data held as double, nullptr if no data is stored or the
...
double *CuVariant::toDoubleP() const {
    if(_d && _d->type == CuVariant::Double && _d->format != CuVariant::Matrix)
        return static_cast<double *> (_d->val);
    else if(_d && _d->type  == CuVariant::Double)
        return matrix_ptr<double>()->raw_data();
    return nullptr;
}

/** \brief Returns the pointer to the data stored as long double, or nullptr
...
long double *CuVariant::toLongDoubleP() const {
    if(_d && _d->type == CuVariant::LongDouble && _d->format != CuVariant::Matrix)
        return static_cast<long double *> (_d->val);
    else if(_d && _d->type == CuVariant::LongDouble)
        return matrix_ptr<long double>()->raw_data();
    return nullptr;
}

/** \brief Returns the pointer to the data stored as unsigned int, or nullptr
 *         if no data is stored or if the data type is wrong
 *
 * Combined with getSize allows to get the stored data in a "C" style (as scalar, if
 * size is 1, or array)
...
unsigned int *CuVariant::toUIntP() const {
    if(_d && _d->type == CuVariant::UInt && _d->format != CuVariant::Matrix) return static_cast<unsigned int *>( _d->val );
    else if(_d && _d->type == CuVariant::UInt) return matrix_ptr<unsigned int>()->raw_data();
    return nullptr;
}

/** \brief Returns the pointer to the data stored as int, or nullptr
 *         if no data is stored or if the data type is wrong
 *
 * Combined with getSize allows to get the stored data in a "C" style (as scalar, if
 * size is 1, or array)
 *
 * @return pointer to the data held as int, nullptr if no data is stored or the
...
int *CuVariant::toIntP() const {
    if(_d && _d->type == CuVariant::Int && _d->format != CuVariant::Matrix) return static_cast<int *>( _d->val );
    else if(_d && _d->type == CuVariant::Int) return matrix_ptr<int>()->raw_data();
    return nullptr;
}

/** \brief Returns the pointer to the data stored as long int, or nullptr
 *         if no data is stored or if the data type is wrong
 *
 * Combined with getSize allows to get the stored data in a "C" style (as scalar, if
 * size is 1, or array)
...
long int *CuVariant::toLongIntP() const {
    if(_d && _d->type == CuVariant::LongInt && _d->format != CuVariant::Matrix)  return static_cast<long int *>(_d->val);
    else if(_d && _d->type == CuVariant::LongInt) return matrix_ptr<long int>()->raw_data();
    return nullptr;
}

long long int *CuVariant::toLongLongIntP() const {
    if(_d && _d->type == CuVariant::LongLongInt && _d->format != CuVariant::Matrix) return static_cast<long long int *>(_d->val);
    else if(_d && _d->type == CuVariant::LongLongInt) return matrix_ptr<long long int>()->raw_data();
    return nullptr;
}

/** \brief Returns the pointer to the data stored as unsigned long int, or nullptr
 *         if no data is stored or if the data type is wrong
 *
 * Combined with getSize allows to get the stored data in a "C" style (as scalar, if
 * size is 1, or array)
...
unsigned long int *CuVariant::toULongIntP() const {
    if(_d && _d->type == CuVariant::LongUInt && _d->format != CuVariant::Matrix) return static_cast<unsigned long int *>(_d->val);
    else if(_d && _d->type == CuVariant::LongUInt) return matrix_ptr<unsigned long int>()->raw_data();
    return nullptr;
}

unsigned long long *CuVariant::toULongLongIntP() const {
    if(_d && _d->type == CuVariant::LongLongUInt && _d->format != CuVariant::Matrix)  return static_cast<unsigned long long int *>(_d->val);
    else if(_d && _d->type == CuVariant::LongLongUInt) return matrix_ptr<unsigned long long int>()->raw_data();
    return nullptr;
}

/** \brief Returns the pointer to the data stored as float, or nullptr
 *         if no data is stored or if the data type is wrong
 *
 * Combined with getSize allows to get the stored data in a "C" style (as scalar, if
 * size is 1, or array)
...
float *CuVariant::toFloatP() const {
    if(_d && _d->type == CuVariant::Float && _d->format != CuVariant::Matrix)  return static_cast<float *>(_d->val);
    else if(_d && _d->type == CuVariant::Float) return matrix_ptr<float>()->raw_data();
    return nullptr;
}

/** \brief Returns the pointer to the data stored as short int, or nullptr
 *         if no data is stored or if the data type is wrong
 *
 * Combined with getSize allows to get the stored data in a "C" style (as scalar, if
 * size is 1, or array)
...
short *CuVariant::toShortP() const {
    if(_d && _d->type == CuVariant::Short && _d->format != CuVariant::Matrix) return static_cast<short int *>(_d->val);
    else if(_d && _d->type == CuVariant::Short) return matrix_ptr<short int>()->raw_data();
    return nullptr;
}

/** \brief Returns the pointer to the data stored as unsigned short int, or nullptr
 *         if no data is stored or if the data type is wrong
 *
 * Combined with getSize allows to get the stored data in a "C" style (as scalar, if
 * size is 1, or array)
...
unsigned short *CuVariant::toUShortP() const {
    if(_d && _d->type == CuVariant::UShort && _d->format != CuVariant::Matrix) return static_cast<unsigned short *>(_d->val);
    else if(_d && _d->type == CuVariant::UShort) return matrix_ptr<unsigned short>()->raw_data();
    return nullptr;
}

/** \brief Returns the pointer to the data stored as a boolean, or nullptr
 *         if no data is stored or if the data type is wrong
 *
 * Combined with getSize allows to get the stored data in a "C" style (as scalar, if
 * size is 1, or array)
...
bool *CuVariant::toBoolP() const {
    if(_d && _d->type == CuVariant::Boolean && _d->format != CuVariant::Matrix) return static_cast<bool *> (_d->val );
    else if (_d && _d->type == CuVariant::Boolean) return matrix_ptr<bool>()->raw_data();
    return nullptr;
}

/** \brief Returns the pointer to the data stored as a char**, or nullptr
 *         if no data is stored or if the data type is wrong
 *
 * Combined with getSize allows to get the stored data in a "C" style (as scalar, if
 * size is 1, or array)
...
char **CuVariant::to_C_charP() const {
    if(_d && _d->type == CuVariant::String && _d->format != CuVariant::Matrix)
        return (char **) _d->val ;
    else if (_d && _d->type == CuVariant::String)
...
char *CuVariant::toCharP() const {
    if(_d && _d->type == CuVariant::Char && _d->format != CuVariant::Matrix)
        return (char *) _d->val ;
    else if(_d && _d->type == CuVariant::Char)
        return matrix_ptr<char>()->raw_data();
    return nullptr;
}

unsigned char *CuVariant::toUCharP() const {
    if(_d && _d->type == CuVariant::UChar && _d->format != CuVariant::Matrix) return (unsigned char *) _d->val;
    else if(_d && _d->type == CuVariant::UChar) return matrix_ptr<unsigned char>()->raw_data();
    return nullptr;
}

void *CuVariant::toVoidP() const {
    if(_d && _d->type == CuVariant::VoidPtr) {
        return _d->val;
    }
    return nullptr;
}
...
/*!
 * \brief return data ptr without any type checking
 * \return internal pointer to raw data
 */
void *CuVariant::data() const {
    return _d ? _d->val : nullptr;
}

...
//
// expand main widget's constructor to pass CumbiaTango
// find CumbiaTango variable name:   CumbiaTango\s*\*\s*([A-Za-z0-9_]*)\s*=\s*new\s+CumbiaTango
...
{
    QString out = input;
    QRegExp cutaRe ("CumbiaTango\\s*\\*\\s*([A-Za-z0-9_]*)\\s*=\\s*new\\s+CumbiaTango");
    m_err = cutaRe.indexIn(out) < 0;
    if(!m_err) {
        QString cuta = cutaRe.cap(1);

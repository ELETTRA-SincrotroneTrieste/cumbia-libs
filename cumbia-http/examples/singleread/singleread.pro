...
!isEmpty(INSTALL_DIR) {
    ...
    message("       execute `make install` to install 'singleread' under $${INSTALL_DIR} ")
    message("-")
} else {
    message("-")
    message("INSTALLATION")
    message("       call qmake INSTALL_DIR=/usr/local/bin to install singleread later with `make install` ")
    message("-")
}

...
CuData CuHttpAuthManager::m_make_error_data(const QString &msg) const {
    CuData ed(TTT::Err, true);  // CuData ed("err", true)
    ed.set(TTT::Message, msg.toStdString()).set(TTT::Src, "auth_manager").putTimestamp();  // set("msg", msg.toStdString(), set("src", "auth_manager")
    if(d->reply)

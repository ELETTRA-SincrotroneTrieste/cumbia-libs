...
QuPlotPanel::QuPlotPanel( QWidget* parent )
    : QTabWidget( parent )
{
    ...
    connect( m_setDataMode, SIGNAL(currentIndexChanged(int)), SLOT(edited()) );
...
QWidget* QuPlotPanel::createPlotTab( QWidget* parent )
{
    ...
    m_updateType->addItem("QuPlotBase.refresh");

    m_setDataMode = new QComboBox(page);
    m_setDataMode->addItem("SeriesData");
    m_setDataMode->addItem("setSamples");
    m_setDataMode->setItemData(0, "Use a custom implementation of QwtSeriesData");
    m_setDataMode->setItemData(1, "Use setSamples(const QVector<double>&x, const QVector<double>&y");
    ...
    layout->addWidget( new QLabel( "Set data mode", page ), row, 0 );
    layout->addWidget( m_setDataMode, row++, 1 );
...
QWidget* QuPlotPanel::createCanvasTab( QWidget* parent )
{
    QWidget* page = new QWidget( parent );
...
QWidget* QuPlotPanel::createCurveTab( QWidget* parent )
{
    QWidget* page = new QWidget( parent );
...
void QuPlotPanel::edited()
{
    const QuPlotSettings s = settings();
    Q_EMIT settingsChanged( s );
}


QuPlotSettings QuPlotPanel::settings() const
{
    QuPlotSettings s;
    ...
    s.curve.functionType = static_cast< QuPlotSettings::FunctionType >(
    ...
    s.updateType = static_cast< QuPlotSettings::UpdateType >( m_updateType->currentIndex() );
    s.setDataType = static_cast<QuPlotSettings::SetDataType>(m_setDataMode->currentIndex());
    return s;
}

void QuPlotPanel::setSettings( const QuPlotSettings& s )
{

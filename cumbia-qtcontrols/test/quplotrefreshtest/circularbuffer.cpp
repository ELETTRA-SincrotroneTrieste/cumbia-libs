...
// to use QuSpectrumPlot setData with QVectors of double
void CircularBuffer::getData(QVector<double> &x, QVector<double> &y) const {
    x.clear();
    y.clear();
    x.reserve(size());
    y.reserve(size());
    for(size_t i = 0; i < size(); i++) {
        const QPointF& p = CircularBuffer::sample(i);
        x << p.x();
        y << p.y();
    }
}

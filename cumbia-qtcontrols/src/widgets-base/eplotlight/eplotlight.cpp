...
void EPlotLight::appendData(const QString& curveName, double *x, double *y, int size)
{
	if(!d_curvesMap.contains(curveName))
	{
	  ...
	  createAndAddCurve(curveName);
        ...
        QuPlotCurve* curve = d_curvesMap.value(curveName);
...
{
  QuPlotCurve *plotCurve = new QuPlotCurve(this, curveName);
  /* and add it to the plot. This will select a color for the curve */
  addCurve(curveName, plotCurve);
}

void EPlotLight::addCurve(const QString& title, QuPlotCurve *curve)
{
...
{
  foreach(QuPlotCurve *c, d_curvesMap.values())
	c->clearData();
  replot();
}
...
void EPlotLight::setCurveStyle(const QString &curveName, QwtPlotCurve::CurveStyle style)
{
  if(d_curvesMap.contains(curveName))
  {
    QuPlotCurve *pCurve = d_curvesMap.value(curveName);
...
QuPlotCurve *EPlotLight::curve(const QString& name)
{
...
QString EPlotLight::curveName(QuPlotCurve *curve)
{
  QString name;
...
QuPlotCurve *EPlotLight::getCurveIgnoreCase(const QString &name)
{
    QuPlotCurve *crv = NULL;
    foreach(QString cname, d_curvesMap.keys())
    {
...
{
    foreach(QuPlotCurve *epc, d_curvesMap.values())

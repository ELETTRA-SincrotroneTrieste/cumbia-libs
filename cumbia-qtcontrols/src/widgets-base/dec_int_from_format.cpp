...
// 	printf("\e[1;36mdecode() format %s\e[0m\n", qstoc(d_format));
	...
	if(pos >= 0) /* integer */
	{
		...
		else
		{
			...
			if(pos >= 0  && capReV2.capturedTexts().size() > 1)
			{
				...
				d_intDefaults = false;
                cuprintf("\e[1;33m* \e[0mwarning: format \"%s\" is not correct, anyway accepting it and setting %d decimals\n",
					qstoc(d_format),  d_intDigits);
                cuprintf("\e[1;33m* \e[0m\e[4mnote\e[0m the correct format is \"%%.%d\"\n", d_intDigits);
			}
		}
	}
	else 
	...
	if(pos < 0)
	{
        cuprintf("\e[1;31m*\e[0m format error in \"%s\"", qstoc(d_format));
		return false;
	}
	else
		return true;

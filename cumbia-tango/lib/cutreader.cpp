...
void CuTReader::onResult(const CuData &data) {
    ...
    if(err) {
        if(event_subscribe_fail)
            printf("CuTReader.onResult: \e[1;35m: polling fallback after \e[1;34;35mevent subscription failure '%s': '%s'\e[0m\n", vtoc2(data, "src"), vtoc2(data, "msg"));
        else if(data.containsKey("ev_except") && data["ev_except"].toBool()) {

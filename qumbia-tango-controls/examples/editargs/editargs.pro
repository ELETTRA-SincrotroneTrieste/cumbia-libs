...
HEADERS += src/editargs.h

# cuuimake runs uic
# FORMS    = src/editargs.ui
...
!isEmpty(INSTALL_DIR) {
    ...
    message("       execute `make install` to install 'editargs' under $${INSTALL_DIR} ")
    message("-")
} else {
    message("-")
    message("INSTALLATION")
    message("       call qmake INSTALL_DIR=/usr/local/bin to install editargs later with `make install` ")
    message("-")
}

...
HEADERS += src/format_test.h

# cuuimake runs uic
# FORMS    = src/format_test.ui
...
!isEmpty(INSTALL_DIR) {
    ...
    message("       execute `make install` to install 'format_test' under $${INSTALL_DIR} ")
    message("-")
} else {
    message("-")
    message("INSTALLATION")
    message("       call qmake INSTALL_DIR=/usr/local/bin to install format_test later with `make install` ")
    message("-")
}

...
void Context::setMode() {
    CuData options(TTT::RefreshMode, static_cast<int>(m_getRefreshMode()));  // CuData options("refresh_mode", static_cast<int>(m_getRefreshMode()
    if(options[TTT::RefreshMode].toInt() == CuTReader::PolledRefresh)  // options["refresh_mode"]
        options[TTT::Period] = findChild<QSpinBox *>()->value();  // options["period"]
...
void Context::onNewData(const CuData &d)
{
    ...
    if(d[TTT::Mode].isValid()) {  // d["mode"]
        le->setText(QString::fromStdString(d[TTT::Mode].toString()));  // d["mode"]
        findChild<QSpinBox *>()->setEnabled(le->text().compare("EVENT", Qt::CaseInsensitive) != 0);
    }
    if(d[TTT::Period].isValid())  // d["period"]
        le->setText(le->text() + ": " + QString::fromStdString(d[TTT::Period].toString()) + "ms");  // d["period"]
}

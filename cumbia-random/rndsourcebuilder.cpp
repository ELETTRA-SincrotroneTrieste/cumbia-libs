 ...
 *
 */
void RndSourceBuilder::fromSource(const QString &src)
{
    QRegularExpressionMatch match;
    // try match size/min/max/period
    QRegularExpression re(".*/(\\d+)/([\\-\\.0-9]+)/([\\-\\.0-9]+)/(\\d+)(?:/(.*))*");
    match = re.match(src);
    ...
    printf("RndSourceBuilder: matching \"%s\" with pat \"%s\": %d OSPIONS %s\n", qstoc(src),
           qstoc(re.pattern()), match.hasMatch(), options.toString().c_str());
}
